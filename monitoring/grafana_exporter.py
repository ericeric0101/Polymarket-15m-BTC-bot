...
class GrafanaMetricsExporter:
    ...
    def __init__(
        ...
        Args:
            host: HTTP bind host for metrics endpoint
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self._update_thread = threading.Thread(target=self._update_loop_sync, daemon=True)
            self._update_thread.start()
            
        except Exception as e:
            logger.error(f"Failed to start metrics server: {e}")
    
    def _update_loop_sync(self) -> None:
        """Periodically update metrics in a background thread."""
...
def get_grafana_exporter() -> GrafanaMetricsExporter:
    ...
    if _grafana_exporter_instance is None:
        host = os.getenv("GRAFANA_EXPORTER_HOST", "127.0.0.1")
        port = int(os.getenv("GRAFANA_EXPORTER_PORT", "8000"))
